...
// en un requisito PermisoRequirement, sin tener que registrar cada permiso a mano
// con AddPolicy(). Cualquier otra política cae al proveedor por defecto.
public sealed class PermisoAuthorizationPolicyProvider(IOptions<AuthorizationOptions> opciones) : IAuthorizationPolicyProvider
{
	...
	public Task<AuthorizationPolicy?> GetPolicyAsync(string policyName)
	{
		...
		{
			var codigoPermiso = policyName[Prefijo.Length..];
			var politica = new AuthorizationPolicyBuilder()
				.RequireAuthenticatedUser()
				.AddRequirements(new PermisoRequirement(codigoPermiso))
				.Build();
